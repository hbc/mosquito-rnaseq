...
Why are 422 and 423 missing? It looks like they are only expressed in MAG, and are expressed
at an extremely high level in MAG.
...
m = merge(melted_counts, old_melted_counts, by.x=c("gene", "sample"), by.y=c("gene", "sample"),
 all=FALSE)
```
...
```{r old-vs-new}
ggplot(m, aes(count.x, count.y)) + geom_point() + xlab("new") + ylab("old") + scale_y_sqrt() + scale_x_sqrt()
```
...
colnames(design) = c("atrium_mated", "atrium_virgin", "body_mated", "body_virgin",
           "head_mated", "head_virgin", "MAG_mated", "MAG_virgin",
            "spermatheca_mated", "spermatheca_virgin")
y = DGEList(counts=counts)
y = calcNormFactors(y)
...
fit2 = eBayes(fit2)
MAG_results = topTable(fit2, n=Inf, p.value=1, coef="MAG")
MAG_results[missing_genes,]
```

So it looks like these genes get knocked out of being significant and it isn't a limma-DESeq2
difference. It looks like adding the other tissues in knocks them out.
...
colnames(design) = c("MAG_mated", "MAG_virgin", "atrium_mated", "atrium_virgin")
...
Quantile normalizing the voom counts also drops them out completely. 
...
fit2 = contrasts.fit(fit, cm)
fit2 = eBayes(fit2)
MAG_results_2 = topTable(fit2, n=Inf, p.value=1, coef="MAG")
MAG_results_2[missing_genes,]
```

There are some reproducible hits swapping normalization, but it is only around half.
There are a bunch of fringe genes that are sensitive to how we do the analysis.


head(m)
ggplot(in_both[in_both$gene %in% missing_genes,], aes(gene, count, fill=run)) + geom_boxplot()

ggplot(in_both[in_both$gene %in% missing_genes,], aes(gene, count, fill=run)) +
 geom_bar(position="dodge", stat="identity") + facet_wrap(~ sample) +
 theme(axis.text.x=element_text(angle=90)) + xlab("") + ylab("# alignments")

in_either = unique(c(rownames(subset(MAG_results, adj.P.Val < 0.1)),
    rownames(subset(MAG_results_2, adj.P.Val < 0.1))))

signew = MAG_results[in_either,]
signew$run = "new"
sigold = MAG_results_2[in_either,]
signew$run = "old"

signew = subset(MAG_results, adj.P.Val < 0.1)[, c("logFC", "AveExpr", "adj.P.Val")]
signew$sig = signew$adj.P.Val < 0.1
sigold = subset(MAG_results_2, adj.P.Val < 0.1)[, c("logFC", "AveExpr", "adj.P.Val")]
sigold$sig = sigold$adj.P.Val < 0.1

intersect(rownames(subset(MAG_results, adj.P.Val < 0.1)),
          rownames(subset(MAG_results_2, adj.P.Val < 0.1))))
