...
#### comparison of spermatheca 24 hours mated with microarray results
spermatheca_24_microarray = read.table("microarray-spermatheca.tsv", sep="\t")
colnames(spermatheca_24_microarray) = c("ensembl_gene_id", "microarray")
spermatheca_24_microarray = subset(spermatheca_24_microarray, ensembl_gene_id != "0")
z = data.frame(spermatheca_time_24_all)
m = merge(spermatheca_24_microarray, z, by.x="ensembl_gene_id", by.y="row.names")

keep = abs(m$lfcMLE) > 1.5 | abs(m$microarray) > 1.5
hmm = m[keep,]
fix_microarray = function(x) {
    if(x > 0) {
        return(x)
    }
    else {
